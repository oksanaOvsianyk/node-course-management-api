...
## 1. Аналіз структури та швидкості завантаження (Lighthouse)
Для аналізу інтерфейсу документації API (Swagger UI) було використано інструмент Google Lighthouse.

* **Результати аналізу:**
![Lighthouse Screenshot](/assets/light.png)
* **Висновок:** Показники продуктивності (Performance) та Core Web Vitals знаходяться на високому рівні, що забезпечує швидкий доступ до документації ендпоінтів.

---

## 2. Тестування навантаження (Autocannon)
Було проведено стрес-тестування системи для перевірки здатності обробляти велику кількість одночасних запитів.

* **Параметри тесту:** 10 одночасних з'єднань протягом 10 секунд.
* **Результати:**
  - Загальна кількість запитів: **21,000**
  - Середня кількість запитів за секунду (Avg Req/Sec): **2,080.81**
  - Середня затримка (Avg Latency): **4.31 ms**
* **Висновок:** Сервер демонструє високу пропускну здатність та стабільну роботу під навантаженням.

---

## 3. Профілювання CPU (Flame Chart)
Під час тесту навантаження було зібрано дані щодо використання процесора через вкладку Performance у Node.js DevTools.

* **Графік навантаження:**
![Flame Chart](/assets/flame.png)
* **Аналіз:** Основний час CPU витрачається на виконання скриптів (Scripting: 8,845 ms). Це відповідає процесам маршрутизації та серіалізації даних.
* **Висновок:** Відсутність довгих затримок (Long Tasks) підтверджує ефективність асинхронної архітектури NestJS.

---

## 4. Аналіз використання оперативної пам'яті (RAM)
Збір даних проводився методами Heap Snapshot та Allocations on Timeline.

* **Стан пам'яті:**
![Memory Analysis](/assets/ram1.png)
![Allocations Timeline](/assets/ram2.png)
* **Аналіз:** Базове споживання пам'яті додатком становить **24.2 MB**. Під час тестування пам'ять виділялася динамічно та вчасно звільнялася збирачем сміття (Garbage Collector).
* **Висновок:** Критичних витоків пам'яті (Memory Leaks) не виявлено.

---

## 5. Аналіз запитів до бази даних (SQL Logs)
Взаємодію з PostgreSQL проаналізовано за допомогою вбудованого логера TypeORM.

* **Логи запитів:**
![Database Logs](/assets/bd.png)
* **Аналіз:** Система використовує оптимізовані SQL-запити з операторами `JOIN` для отримання пов'язаних сутностей (курси, лекції, користувачі).
* **Висновок:** Взаємодія з БД побудована ефективно, що мінімізує кількість звернень до PostgreSQL на Render.

---

## Загальний висновок
Проведене профілювання підтвердило високу продуктивність системи **Course Management API**. Всі компоненти (CPU, RAM, DB) працюють збалансовано, забезпечуючи швидку відповідь сервера навіть при високій інтенсивності запитів.
